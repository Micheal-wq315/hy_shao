...
static void GimbalSessionStart()
{
    ...
    if(vision_recv_data_r.target_state == NO_TARGET)
    {
        ...
        vision_l_pitch_tar = pitch_l_motor->measure.total_angle + (gimbal_cmd_recv.pitch+0.125)*40*(-1);

        // 1. 准备原始输入数据
        float raw_yaw_input = gimbal_cmd_recv.yaw * RAD_2_DEGREE *0.4; // 将输入转换为角度误差
        float raw_pitch_input = (gimbal_cmd_recv.pitch + 0.087f) * 20 *(-1.0f);
        ...
        //Gimbal_Base追击逻辑，使用滤波后的yaw输入，增强追击稳定性
        //YawTrackingControl();
    }
