...
static Vision_Recv_s *vision_recv_data_; // 视觉接收数据指针,初始化时返回
...
static void VisionRadaControlSet()
{
    ...
    else if(vision_recv_data_->fire_mode == AUTO_FIRE) 
    {
        shoot_cmd_send.friction_mode = FRICTION_ON;

        shoot_cmd_send.shoot_rate = 5;
        if(vision_recv_data_->yaw<0.05f && vision_recv_data_->yaw > -0.05f)
            shoot_cmd_send.load_mode = LOAD_BURSTFIRE;
        else
            shoot_cmd_send.load_mode = LOAD_STOP;
...
void RobotCMDTask()
{
   // BMI088Acquire(bmi088_test,&bmi088_data) ;
    ...
    PubPushMessage(vision_recv_r_data_pub, (void *)vision_recv_data_);
}
